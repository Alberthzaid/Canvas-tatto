      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="masonry-item space-y-4">
          {column1.map((img, idx) => (
            <div
              key={idx}
              className={`${img.aspectRatio} bg-zinc-900 rounded-sm overflow-hidden`}
              data-alt={`Dark aesthetic tattoo masterpiece ${idx + 1}`}
            >
              <img
                alt={img.alt}
        ...
        <div className="masonry-item space-y-4 pt-12">
          {column2.map((img, idx) => (
            <div
              key={idx}
              className={`${img.aspectRatio} bg-zinc-900 rounded-sm overflow-hidden`}
              data-alt={`Dark aesthetic tattoo masterpiece ${idx + 3}`}
            >
              <img
                alt={img.alt}
                className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700"
        ...
        <div className="masonry-item space-y-4">
          {column3.map((img, idx) => (
            <div
              key={idx}
              className={`${img.aspectRatio} bg-zinc-900 rounded-sm overflow-hidden`}
              data-alt={`Dark aesthetic tattoo masterpiece ${idx + 5}`}
            >
              <img
                alt={img.alt}
                className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700"
        ...
        <div className="masonry-item space-y-4 pt-8">
          {column4.map((img, idx) => (
            <div
              key={idx}
              className={`${img.aspectRatio} bg-zinc-900 rounded-sm overflow-hidden`}
              data-alt={`Dark aesthetic tattoo masterpiece ${idx + 7}`}
            >
              <img
                alt={img.alt}
                className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700"
